 ...
 * 获取动态数据 按钮 第二层adapter 产前待产
 */

public class AntenatalExpectantSoleListAdapter extends RecyclerCommonAdapter<AntenatalExpectant.DictsBean> {

    private int mSelectedPos = -1;//实现单选, 保存上次选中的位置
    private SparseBooleanArray mSinglePositions;  //保存单选 数据
    private RecyclerView mRv;
    private AntenatalExpectantSoleListAdapter.OnItemClickListner listner;

    public AntenatalExpectantSoleListAdapter(AppCompatActivity context, List<AntenatalExpectant.DictsBean> datas) {
    ...
    public void setListner(AntenatalExpectantSoleListAdapter.OnItemClickListner listner) {
