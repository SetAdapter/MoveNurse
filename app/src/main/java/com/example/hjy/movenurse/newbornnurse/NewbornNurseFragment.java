...
public class NewbornNurseFragment extends BaseFragment {
    ...
    NewbornNurseAdapter mAdapter;

    @Override
    protected int getContentLayout() {
        return R.layout.newborn_nurse;
    }
    ...
    private void initRv() {
        List<NewBornNurseRecordBean> data = new ArrayList<>();
        ...
        mAdapter = new NewbornNurseAdapter(mContext, data);
    ...
    private void getNursingRecodes() {
        ...
        NetRequest.getInstens().requestDate(params, Api.NeonatalCareRecodes2, false, new NetCallBack<ArrayList<NewBornNurseRecordBean>>(mContext, R.string.loading_get) {
            @Override
            public void onSuccess(ArrayList<NewBornNurseRecordBean> beans) {
                if (null != beans) {
                    tvHospitalizationDate.setText(beans.get(0).getNCNursing().getBirthDate());
                }
                mAdapter.setmDatas(beans);
    ...
    public void onClick(View view) {
        ...
        switch (view.getId()) {
            case R.id.tvEdit://编辑按钮
                JumpUtils.jump(mContext, NewbornNurseActivity.class, null);
                break;
        }
    }
}
