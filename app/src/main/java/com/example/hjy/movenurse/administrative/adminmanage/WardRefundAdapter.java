...
/**
 * 行政管理 病区退非 数据列表 adapter
 * <p/>Created by fangs on 2017/9/30.
 */
public class WardRefundAdapter extends RecyclerCommonAdapter<PainbBillDetailsBean.ResultDataBean> implements Filterable{

    public WardRefundAdapter(AppCompatActivity context, List<PainbBillDetailsBean.ResultDataBean> datas) {
        super(context, R.layout.item_refund_grid, datas);
    }
    ...
    public void convert(ViewHolder holder, PainbBillDetailsBean.ResultDataBean resultDataBean, int position) {
        ...
        holder.setText(R.id.tv_name, "项目名称:  "+resultDataBean.getProjectName());//项目名称
//        holder.setText(R.id.tv_count, "数量:  "+resultDataBean.getAmount());//数量
        holder.setText(R.id.tv_price, "单价:  "+resultDataBean.getPrice());//单价
        holder.setText(R.id.tv_feeAmount, "应收金额:  "+resultDataBean.getFeeAmount());//单位
        holder.setText(R.id.tv_time, "计费时间:  "+resultDataBean.getPSDate());//计费时间

        //填写退药数量
        EditText et_count= (EditText) holder.itemView.findViewById(R.id.et_count);
        ...
        CheckBox select = holder.getView(R.id.refund_select);
